...
const requirementSessionStorage = (
  requirement: TypeSessionStorageRequirement,
) => {
...
const requirementNode = async (
  requirement: TypeNodeRequirement,
): Promise<boolean> => {
  const booleanPromises = requirement.data.children.map((childData) => {
    const requirementStrategies = {
      ...
      localStorage: () =>
        requirementLocalStorage(childData as TypeLocalStorageRequirement),
      node: () => requirementNode(childData as TypeNodeRequirement),
      sessionStorage: () =>
        requirementSessionStorage(childData as TypeSessionStorageRequirement),
      queryParam: () =>
        requirementQueryParam(childData as TypeQueryParamRequirement),
